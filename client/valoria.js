...
try {
  ...
  class Valoria {
    ...
    setup = async () => {
      ...
      return new Promise(async (res, rej) => {
        ...
        try {
          ...
          console.log("joining or creating group");
    ...
    loadCredentials = async (credentials="", password="") => {
      ...
      return new Promise(async (res, rej) => {
        try {
          ...
          const ecdhPrivKey = await crypto.subtle.unwrapKey(
          ...
          const ecdsaPrivKey = await crypto.subtle.unwrapKey(
    ...
    generateCredentials = async (password="") => {
      const self = this;
      return new Promise(async(res, rej) => {
        self.ecdsa = await crypto.subtle.generateKey(
          {name: "ECDSA", namedCurve: "P-384"}, true, ["sign", "verify"]
        );
        self.ecdh = await crypto.subtle.generateKey(
          {name: "ECDH", namedCurve: "P-384"}, true, ["deriveKey", "deriveBits"]
        );
        const ecdsaPubHash = await crypto.subtle.digest("SHA-256", await crypto.subtle.exportKey("raw", self.ecdsa.publicKey));
        self.id = buf2hex(ecdsaPubHash).substr(24, 64)
        const ecdsaSalt = crypto.getRandomValues(new Uint8Array(16))
        const ecdsaIv = crypto.getRandomValues(new Uint8Array(12))
        const ecdhSalt = crypto.getRandomValues(new Uint8Array(16))
        const ecdhIv = crypto.getRandomValues(new Uint8Array(12));
        if(!password) password = await arrayBufferToBase64(crypto.getRandomValues(new Uint8Array(32)));
        const encryptKey = await crypto.subtle.importKey(
          "raw",
        ...
        ecdsaToSave.publicKey = await arrayBufferToBase64(await crypto.subtle.exportKey("raw", self.ecdsa.publicKey));
        ...
        ecdhToSave.publicKey = await arrayBufferToBase64(await crypto.subtle.exportKey("raw", self.ecdh.publicKey));
    ...
    requestNewGroup(){
      ...
      return new Promise(async(res, rej) => {
        try {
          ...
          console.log("Sent create group request to " + url + " for " + groupIndex);
        } catch(e){
          rej();
        }
      })
        ...
        ws.onmessage = async (e) => {
          ...
          switch (d.event) {
            ...
            case "Got rtc description":
              await self.handleGotRtcDescription(ws, d.data);
              break;
    ...
    handleVerifyUrlRequest = async (ws, data) => {
      ...
      return new Promise(async( res, rej) => {
        try {
          ...
          self.verifying[data.url] = buf2hex(crypto.getRandomValues(new Uint8Array(32)))
          await new Promise(async(res, rej) => {
        ...
        try {
          ...
          if(!self.peers[url]){
            ...
            self.peers[url].onnegotiationneeded = async options => {
              if(!self.peers[url] || !self.url) return;
              try {
    ...
    async handleGotRtcCandidate(ws, data){
      ...
      try {
        ...
        console.log("Adding candidate for " + data.url);
