...
function TouchControls(parent, camera, avatar, options) {
  ...
  parent.addEventListener('touchmove', function(e) { e.preventDefault(); }, { passive:false });
  ...
  this.camera.dirTarget.position.set(this.camera.position.x, this.camera.position.y, this.camera.position.z + 3);
    ...
    self.rotateEnd.set( self.rotateStart.x + event.detail.deltaX, self.rotateStart.y - event.detail.deltaY );
    self.rotateDelta.subVectors( self.rotateEnd, self.rotateStart ).multiplyScalar( self.rotateSpeed );
    ...
    self.camera.dirTarget.position.set(self.camera.position.x + (self.avatar.move.left * 10), self.camera.position.y, self.camera.position.z + (self.avatar.move.forward * 20) - 2);
    ...
    if(
      ...
      self.avatar.lookAt(self.avatar.position.x - (pos.x - self.avatar.position.x), self.avatar.position.y, self.avatar.position.z - (pos.z - self.avatar.position.z));
    ...
    if(self.avatar.jump){
      self.setAction(self.avatar.animationActions[2], 1);
    } else if(!self.avatar.jump){
      if(isMoving){
        self.setAction(self.avatar.animationActions[3], 1);
      } else {
        if(self.avatar.dancing){
          self.setAction(self.avatar.animationActions[0], 1);
        } else {
          self.setAction(self.avatar.animationActions[1], 1);
        }
      }
    }
    ...
    self.camera.lookAt( self.avatar.position.x, self.avatar.position.y + 1, self.avatar.position.z );
...
function MovementPad(container) {
	...
	self.movementPad.css({
		top: self.container.height() + self.container.position().top - self.region.outerHeight() - 10,
		left: 20
	});
